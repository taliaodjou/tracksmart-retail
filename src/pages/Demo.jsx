...
import {
  Plus, Search, X, ScanLine, Sparkles, Package, AlertTriangle, XCircle,
  ChevronRight, ChevronDown, CheckCircle2, Loader2, Tag, Layers, RefreshCw,
  TrendingDown, TrendingUp, PackageX, Flame, BarChart2, ShoppingCart,
  FileText, Download, Send, Building2, Mail, Phone, SlidersHorizontal, Bell,
...
const SEED = [
  ...
  // Discarded in previous months for analytics
...
// ═══════════ TAB: DASHBOARD ═══════════
function DashboardTab({ products, addProduct, deleteProduct, handleProductAction, showScanner, setShowScanner, quickAdd, setQuickAdd, nextId, setNextId }) {
...
function AnalyticsTab({ products }) {
  ...
  const lossTrend = prevMonth?.totalLoss>0?(((currentMonth?.totalLoss-prevMonth.totalLoss)/prevMonth.totalLoss)*100).toFixed(0):null;
  const expiredTrend = prevMonth?.expiredCount>0?currentMonth?.expiredCount-prevMonth.expiredCount:null;

  const categoryStats = useMemo(()=>{const map={};products.forEach(p=>{if(!p.category)return;if(!map[p.category])map[p.category]={count:0,loss:0};if(isDiscarded(p)||getProductStatus(p.expiration_date)==='expired'){map[p.category].count++;map[p.category].loss+=(p.quantity_thrown||0)*(p.price_chf||0)}});return Object.entries(map).map(([cat,data])=>({cat,label:categoryKeys[cat]||cat,...data})).sort((a,b)=>b.loss-a.loss).slice(0,6)},[products]);
  const rayonStats = useMemo(()=>{const map={};products.forEach(p=>{if(!p.rayon)return;if(!map[p.rayon])map[p.rayon]={count:0,loss:0};if(isDiscarded(p)||getProductStatus(p.expiration_date)==='expired'){map[p.rayon].count++;map[p.rayon].loss+=(p.quantity_thrown||0)*(p.price_chf||0)}});return Object.entries(map).map(([rayon,data])=>({rayon:`R${rayon}`,...data})).sort((a,b)=>b.loss-a.loss).slice(0,8)},[products]);
  const topProblematic = useMemo(()=>products.filter(p=>(isDiscarded(p)||getProductStatus(p.expiration_date)==='expired')&&p.price_chf).sort((a,b)=>((b.quantity_thrown||0)*(b.price_chf||0))-((a.quantity_thrown||0)*(a.price_chf||0))).slice(0,5),[products]);
  ...
  const totalThrown = products.reduce((s,p)=>s+(p.quantity_thrown||0),0);
  const urgentNow = products.filter(p=>!isDiscarded(p)&&getProductStatus(p.expiration_date)==='urgent').length;
  ...
  return (<div className="space-y-6 sm:space-y-8">
    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
      {[{icon:<TrendingDown className="w-5 h-5 text-primary"/>,value:`CHF ${totalLoss.toFixed(2)}`,label:'Pertes totales',bg:'bg-primary/10'},{icon:<PackageX className="w-5 h-5 text-red-500"/>,value:totalExpired,label:'Produits expirés',bg:'bg-red-50'},{icon:<Flame className="w-5 h-5 text-orange-500"/>,value:totalThrown,label:'Quantité jetée',bg:'bg-orange-50'},{icon:<AlertTriangle className="w-5 h-5 text-yellow-500"/>,value:urgentNow,label:'Urgents maintenant',bg:'bg-yellow-50'}].map((k,i)=><div key={i} className="bg-white rounded-2xl border shadow-sm p-5 flex flex-col gap-3"><div className={`w-10 h-10 rounded-xl ${k.bg} flex items-center justify-center`}>{k.icon}</div><div><p className="text-xl font-bold">{k.value}</p><p className="text-xs text-muted-foreground mt-0.5">{k.label}</p></div></div>)}
    </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-white rounded-2xl border shadow-sm p-5"><h3 className="font-semibold text-sm mb-4">Quantité jetée par mois</h3><ResponsiveContainer width="100%" height={220}><BarChart data={monthlyData} margin={{top:5,right:10,bottom:5,left:0}}><CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0"/><XAxis dataKey="label" tick={{fontSize:11}}/><YAxis tick={{fontSize:11}}/><Tooltip formatter={v=>[v,'Unités jetées']}/><Bar dataKey="totalThrown" radius={[6,6,0,0]}>{monthlyData.map((_,i)=><Cell key={i} fill={i===monthlyData.length-1?'#f97316':'#fed7aa'}/>)}</Bar></BarChart></ResponsiveContainer></div>
      <div className="bg-white rounded-2xl border shadow-sm p-5"><h3 className="font-semibold text-sm mb-4">Pertes par catégorie (CHF)</h3>{categoryStats.length===0?<div className="h-[220px] flex items-center justify-center text-sm text-muted-foreground">Aucune donnée</div>:<ResponsiveContainer width="100%" height={220}><BarChart data={categoryStats} layout="vertical" margin={{top:5,right:30,bottom:5,left:60}}><CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" horizontal={false}/><XAxis type="number" tick={{fontSize:11}}/><YAxis dataKey="label" type="category" tick={{fontSize:11}} width={55}/><Tooltip formatter={v=>[`CHF ${v.toFixed(2)}`,'Pertes']}/><Bar dataKey="loss" radius={[0,6,6,0]} fill="hsl(var(--primary) / 0.7)"/></BarChart></ResponsiveContainer>}</div>
    </div>
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="bg-white rounded-2xl p-6 shadow-sm border"><h3 className="font-semibold mb-4 flex items-center gap-2"><AlertTriangle className="w-4 h-4 text-orange-500"/>Rayons à surveiller</h3>{rayonStats.length===0?<p className="text-sm text-muted-foreground">Aucun rayon problématique.</p>:<div className="space-y-2">{rayonStats.map((r,i)=><div key={r.rayon} className="flex items-center justify-between p-3 rounded-xl bg-secondary/50"><div className="flex items-center gap-3"><span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white ${i===0?'bg-red-500':i===1?'bg-orange-500':'bg-yellow-500'}`}>{i+1}</span><span className="font-medium text-sm">Rayon {r.rayon.slice(1)}</span></div><div className="text-right"><p className="text-sm font-semibold">CHF {r.loss.toFixed(2)}</p><p className="text-xs text-muted-foreground">{r.count} expirés</p></div></div>)}</div>}</div>
      <div className="bg-white rounded-2xl p-6 shadow-sm border"><h3 className="font-semibold mb-4 flex items-center gap-2"><Flame className="w-4 h-4 text-red-500"/>Produits les plus problématiques</h3>{topProblematic.length===0?<p className="text-sm text-muted-foreground">Aucune perte enregistrée.</p>:<div className="space-y-2">{topProblematic.map((p,i)=>{const loss=(p.quantity_thrown||0)*(p.price_chf||0);return <div key={p._id} className="flex items-center justify-between p-3 rounded-xl bg-secondary/50"><div className="flex items-center gap-3"><span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white ${i===0?'bg-red-500':i===1?'bg-orange-500':'bg-yellow-500'}`}>{i+1}</span><div><p className="text-sm font-medium">{p.name}</p>{p.marque&&<p className="text-xs text-muted-foreground">{p.marque}</p>}</div></div><div className="text-right"><p className="text-sm font-semibold text-red-600">CHF {loss.toFixed(2)}</p><p className="text-xs text-muted-foreground">{p.quantity_thrown} jetés</p></div></div>})}</div>}</div>
    </div>
  </div>);
}
...
function OrdersTab({ products }) {
  const sortByUrgency = (list) => [...list].sort((a,b)=>{const da=a.expiration_date?new Date(a.expiration_date).getTime():Infinity;const db=b.expiration_date?new Date(b.expiration_date).getTime():Infinity;return da-db});
  ...
  const toggleAll = () => {if(selectedIds.size===eligible.length)setSelectedIds(new Set());else setSelectedIds(new Set(eligible.map(p=>p._id)))};
  const selected = eligible.filter(p=>selectedIds.has(p._id));

  const statusBadge = (p) => {const s=getProductStatus(p.expiration_date);const m={expired:'bg-red-100 text-red-700',urgent:'bg-orange-100 text-orange-700',soon:'bg-yellow-100 text-yellow-700',ok:'bg-green-100 text-green-700'};return <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${m[s]}`}>{s==='expired'?'Expiré':s==='urgent'?'Urgent':s==='soon'?'Bientôt':'OK'}</span>};

  const filtered = useMemo(()=>eligible.filter(p=>{if(search.trim()&&!p.name?.toLowerCase().includes(search.toLowerCase())&&!p.marque?.toLowerCase().includes(search.toLowerCase()))return false;if(categoryFilter!=='all'&&p.category!==categoryFilter)return false;if(rayonFilter!=='all'&&p.rayon!==rayonFilter)return false;if(statusFilter!=='all'){const s=getProductStatus(p.expiration_date);if(statusFilter==='a_recommander'&&p.action!=='a_recommander')return false;if(statusFilter!=='a_recommander'&&s!==statusFilter)return false}return true}),[eligible,search,categoryFilter,rayonFilter,statusFilter]);

  const handleDemoSend = () => {if(selected.length===0||!supplier.email)return;setSent(true);setTimeout(()=>setSent(false),4000)};
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 space-y-4">
        <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
          <div className="px-4 py-3 border-b space-y-2.5"><div className="flex items-center justify-between"><h2 className="font-semibold text-sm flex items-center gap-2"><Package className="w-4 h-4 text-primary"/>Produits à commander{eligible.length>0&&<span className="bg-primary/10 text-primary text-xs font-semibold px-2 py-0.5 rounded-full">{eligible.length}</span>}</h2><button className="text-xs text-primary font-medium" onClick={toggleAll}>{selectedIds.size===eligible.length?'Tout désélectionner':'Tout sélectionner'}</button></div>
            <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground"/><Input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Rechercher un produit..." className="pl-8 h-9 text-sm rounded-full"/></div>
            <div className="flex gap-2 items-center overflow-x-auto pb-0.5">
              <Select value={statusFilter} onValueChange={setStatusFilter}><SelectTrigger className={`h-8 text-xs rounded-full flex-shrink-0 w-auto px-3 ${statusFilter!=='all'?'border-primary text-primary':''}`}><SelectValue placeholder="Statut"/></SelectTrigger><SelectContent><SelectItem value="all">Tous les statuts</SelectItem><SelectItem value="expired">Expirés</SelectItem><SelectItem value="urgent">Urgents</SelectItem><SelectItem value="soon">Bientôt</SelectItem><SelectItem value="a_recommander">À recommander</SelectItem></SelectContent></Select>
              <Select value={categoryFilter} onValueChange={setCategoryFilter}><SelectTrigger className={`h-8 text-xs rounded-full flex-shrink-0 w-auto px-3 ${categoryFilter!=='all'?'border-primary text-primary':''}`}><SelectValue placeholder="Catégorie"/></SelectTrigger><SelectContent><SelectItem value="all">Toutes catégories</SelectItem>{Object.entries(categoryKeys).map(([v,k])=><SelectItem key={v} value={v}>{k}</SelectItem>)}</SelectContent></Select>
              <Select value={rayonFilter} onValueChange={setRayonFilter}><SelectTrigger className={`h-8 text-xs rounded-full flex-shrink-0 w-auto px-3 ${rayonFilter!=='all'?'border-primary text-primary':''}`}><SelectValue placeholder="Rayon"/></SelectTrigger><SelectContent><SelectItem value="all">Tous les rayons</SelectItem>{Object.keys(rayonKeys).map(r=><SelectItem key={r} value={r}>Rayon {r}</SelectItem>)}</SelectContent></Select>
            </div>
          </div>
          {eligible.length===0?<div className="text-center py-16 text-muted-foreground text-sm"><Package className="w-10 h-10 mx-auto mb-3 opacity-30"/><p>Aucun produit à commander</p></div>:<div className="divide-y">
            {expiredUrgent.length>0&&<div className="px-5 py-2 bg-red-50 border-b border-red-100 flex items-center gap-2"><AlertTriangle className="w-3.5 h-3.5 text-red-600"/><span className="text-xs font-semibold text-red-700">Expirés & Urgents ({expiredUrgent.length})</span></div>}
            {expiredUrgent.filter(p=>{if(search.trim()&&!p.name?.toLowerCase().includes(search.toLowerCase())&&!p.marque?.toLowerCase().includes(search.toLowerCase()))return false;if(categoryFilter!=='all'&&p.category!==categoryFilter)return false;if(rayonFilter!=='all'&&p.rayon!==rayonFilter)return false;if(statusFilter!=='all'&&statusFilter!=='expired'&&statusFilter!=='urgent')return false;return true}).map(p=>{const isSel=selectedIds.has(p._id);return <div key={p._id} className={`px-4 py-3 flex flex-col gap-2 transition-colors cursor-pointer ${isSel?'bg-primary/5 border-l-2 border-primary':'border-l-2 border-transparent'}`} onClick={()=>toggle(p._id)}><div className="flex items-start gap-3"><Checkbox checked={isSel} onCheckedChange={()=>toggle(p._id)} className="mt-0.5" onClick={e=>e.stopPropagation()}/><div className="flex-1 min-w-0"><div className="flex items-center gap-1.5 flex-wrap"><span className="font-semibold text-sm">{p.name}</span>{p.marque&&<span className="text-xs text-muted-foreground">· {p.marque}</span>}</div><div className="flex items-center gap-1.5 flex-wrap mt-1">{statusBadge(p)}{p.action==='a_recommander'&&<span className="text-xs px-2 py-0.5 rounded-full bg-primary/10 text-primary font-medium border border-primary/20">À recommander</span>}</div><div className="text-xs text-muted-foreground mt-1 flex flex-wrap gap-x-2 gap-y-0.5">{p.category&&<span>{categoryKeys[p.category]||p.category}</span>}{p.rayon&&<span>Rayon {p.rayon}</span>}{p.expiration_date&&<span className={getDaysRemaining(p.expiration_date)<0?'text-red-500 font-medium':getDaysRemaining(p.expiration_date)<=7?'text-orange-500 font-medium':''}>DLC: {p.expiration_date} ({getDaysRemaining(p.expiration_date)}j)</span>}</div></div></div>{isSel&&<div className="flex items-center gap-2 pl-6 pt-1" onClick={e=>e.stopPropagation()}><div><label className="text-xs text-muted-foreground block mb-1">Qté</label><Input type="number" min="1" value={quantities[p._id]||1} onChange={e=>setQuantities(q=>({...q,[p._id]:parseInt(e.target.value)||1}))} className="h-8 w-16 text-sm text-center"/></div><div className="flex-1"><label className="text-xs text-muted-foreground block mb-1">Note</label><Input value={notes[p._id]||''} onChange={e=>setNotes(n=>({...n,[p._id]:e.target.value}))} className="h-8 w-full text-sm" placeholder="optionnel"/></div></div>}</div>})}
            {toReorder.length>0&&<div className="px-5 py-2 bg-primary/5 border-b border-primary/10 flex items-center gap-2"><Package className="w-3.5 h-3.5 text-primary"/><span className="text-xs font-semibold text-primary">À recommander ({toReorder.length})</span></div>}
            {toReorder.filter(p=>{if(search.trim()&&!p.name?.toLowerCase().includes(search.toLowerCase())&&!p.marque?.toLowerCase().includes(search.toLowerCase()))return false;if(categoryFilter!=='all'&&p.category!==categoryFilter)return false;if(rayonFilter!=='all'&&p.rayon!==rayonFilter)return false;if(statusFilter==='expired'||statusFilter==='urgent')return false;return true}).map(p=>{const isSel=selectedIds.has(p._id);return <div key={p._id} className={`px-4 py-3 flex flex-col gap-2 transition-colors cursor-pointer ${isSel?'bg-primary/5 border-l-2 border-primary':'border-l-2 border-transparent'}`} onClick={()=>toggle(p._id)}><div className="flex items-start gap-3"><Checkbox checked={isSel} onCheckedChange={()=>toggle(p._id)} className="mt-0.5" onClick={e=>e.stopPropagation()}/><div className="flex-1 min-w-0"><div className="flex items-center gap-1.5 flex-wrap"><span className="font-semibold text-sm">{p.name}</span>{p.marque&&<span className="text-xs text-muted-foreground">· {p.marque}</span>}</div><div className="flex items-center gap-1.5 flex-wrap mt-1">{statusBadge(p)}{p.action==='a_recommander'&&<span className="text-xs px-2 py-0.5 rounded-full bg-primary/10 text-primary font-medium border border-primary/20">À recommander</span>}</div><div className="text-xs text-muted-foreground mt-1 flex flex-wrap gap-x-2 gap-y-0.5">{p.category&&<span>{categoryKeys[p.category]||p.category}</span>}{p.rayon&&<span>Rayon {p.rayon}</span>}{p.expiration_date&&<span className={getDaysRemaining(p.expiration_date)<0?'text-red-500 font-medium':getDaysRemaining(p.expiration_date)<=7?'text-orange-500 font-medium':''}>DLC: {p.expiration_date} ({getDaysRemaining(p.expiration_date)}j)</span>}</div></div></div>{isSel&&<div className="flex items-center gap-2 pl-6 pt-1" onClick={e=>e.stopPropagation()}><div><label className="text-xs text-muted-foreground block mb-1">Qté</label><Input type="number" min="1" value={quantities[p._id]||1} onChange={e=>setQuantities(q=>({...q,[p._id]:parseInt(e.target.value)||1}))} className="h-8 w-16 text-sm text-center"/></div><div className="flex-1"><label className="text-xs text-muted-foreground block mb-1">Note</label><Input value={notes[p._id]||''} onChange={e=>setNotes(n=>({...n,[p._id]:e.target.value}))} className="h-8 w-full text-sm" placeholder="optionnel"/></div></div>}</div>})}
          </div>}
        </div>
      </div>
      <div className="space-y-4">
        {selectedIds.size>0&&<div className="bg-white rounded-2xl shadow-sm border p-4"><h2 className="font-semibold text-sm flex items-center gap-2 mb-3"><FileText className="w-4 h-4 text-primary"/>Résumé<span className="ml-auto bg-primary text-primary-foreground text-xs font-bold px-2 py-0.5 rounded-full">{selectedIds.size}</span></h2><div className="space-y-1.5"><div className="flex justify-between text-sm"><span className="text-muted-foreground">Quantité totale</span><span className="font-semibold">{selected.reduce((s,p)=>s+(quantities[p._id]||1),0)} unités</span></div><div className="border-t pt-2 mt-2 space-y-1">{selected.slice(0,4).map(p=><div key={p._id} className="flex justify-between text-xs"><span className="truncate max-w-[160px]">{p.name}</span><span className="text-muted-foreground ml-2">×{quantities[p._id]||1}</span></div>)}{selected.length>4&&<p className="text-xs text-muted-foreground">+{selected.length-4} autres…</p>}</div></div></div>}
        <div className="bg-white rounded-2xl shadow-sm border p-4"><h2 className="font-semibold text-sm flex items-center gap-2 mb-3"><Building2 className="w-4 h-4 text-primary"/>Fournisseur</h2><div className="space-y-2.5">
          <div><label className="text-xs font-medium text-muted-foreground block mb-1">Nom</label><Input value={supplier.name} onChange={e=>setSupplier(s=>({...s,name:e.target.value}))} placeholder="Ex: Metro Cash & Carry" className="text-sm h-9"/></div>
          <div><label className="text-xs font-medium text-muted-foreground block mb-1">Contact</label><Input value={supplier.contact} onChange={e=>setSupplier(s=>({...s,contact:e.target.value}))} placeholder="Nom du contact" className="text-sm h-9"/></div>
          <div><label className="text-xs font-medium text-muted-foreground block mb-1 flex items-center gap-1"><Mail className="w-3 h-3"/>Email</label><Input type="email" value={supplier.email} onChange={e=>setSupplier(s=>({...s,email:e.target.value}))} placeholder="fournisseur@example.com" className="text-sm h-9"/></div>
          <div className="grid grid-cols-2 gap-2"><div><label className="text-xs font-medium text-muted-foreground block mb-1 flex items-center gap-1"><Phone className="w-3 h-3"/>Téléphone</label><Input value={supplier.phone} onChange={e=>setSupplier(s=>({...s,phone:e.target.value}))} placeholder="+41 XX XXX XX XX" className="text-sm h-9"/></div><div><label className="text-xs font-medium text-muted-foreground block mb-1">Adresse</label><Input value={supplier.address} onChange={e=>setSupplier(s=>({...s,address:e.target.value}))} placeholder="Adresse" className="text-sm h-9"/></div></div>
        </div></div>
        <div className="flex gap-2">
          <Button variant="outline" className="flex-1 gap-2 rounded-xl h-11" disabled={selectedIds.size===0}><Download className="w-4 h-4"/>PDF</Button>
          <Button className="flex-[2] gap-2 rounded-xl h-11 px-5" onClick={handleDemoSend} disabled={selectedIds.size===0}>{sent?<><CheckCircle2 className="w-4 h-4"/>Envoyé!</>:<><Send className="w-4 h-4"/>Envoyer</>}</Button>
        </div>
      </div>
    </div>
  </div>);
...
export default function Demo() {
  ...
  const addProduct = (data, updateId) => {
    if (updateId) {
      ...
      setProducts(prev=>[{_id:nextId,...data,added_by_name:'Vous (demo)',discarded:false,discarded_at:null,action:null,quantity_thrown:0},...prev]);
      setNextId(n=>n+1);
    }
    setQuickAdd(null);
  };
  ...
  return (
    <div className="min-h-screen pb-20 sm:pb-0 pt-0" style={{backgroundColor:'#f5f5f5',color:'#1a1a1a'}}>
      ...
      <header className="bg-white border-b sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 h-14 sm:h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-0.5 sm:gap-1">
            {tabs.map(t => (
              <button key={t.key} onClick={()=>setTab(t.key)} className={`flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 sm:py-1.5 rounded-full text-xs sm:text-sm font-medium transition-colors ${tab===t.key?'bg-primary text-primary-foreground':'text-muted-foreground hover:text-foreground hover:bg-secondary'}`}>
                <t.icon className="w-3 h-3 sm:w-3.5 sm:h-3.5"/><span className="hidden sm:inline">{t.label}</span>
              </button>
            ))}
          </div>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={()=>setShowScanner(true)} className="rounded-full gap-1 sm:gap-2 text-xs sm:text-sm h-8 sm:h-9 px-2.5 sm:px-3">
              <ScanLine className="w-3.5 h-3.5 sm:w-4 sm:h-4"/> Scanner
            </Button>
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 pt-4 sm:pt-6 pb-6 sm:pb-8">
        {tab==='dashboard'&&<DashboardTab products={products} addProduct={addProduct} deleteProduct={deleteProduct} handleProductAction={handleProductAction} showScanner={showScanner} setShowScanner={setShowScanner} quickAdd={quickAdd} setQuickAdd={setQuickAdd} nextId={nextId} setNextId={setNextId}/>}
