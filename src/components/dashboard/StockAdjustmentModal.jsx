...
const lossReasons = ['Produit périmé', 'Produit abîmé', 'Casse', 'Erreur de comptage', 'Erreur de réception', 'Retour fournisseur', 'Vol ou perte', 'Don / dégustation', 'Retrait qualité', 'Autre'];
...
export default function StockAdjustmentModal({ products = [], product, entry, onProductChange, onScan, onClose, onSubmit, saving }) {
  ...
  const justification = movementType === 'perte' ? (reason === 'Autre' ? customReason : reason) : '';
  const canSave = product && Number(quantity) > 0 && movementType && date;
    ...
    <div className="fixed inset-0 z-[95] bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-white w-full sm:max-w-lg rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border/40">
          <div><p className="text-xs uppercase tracking-wide text-muted-foreground font-semibold">Gestion de stock</p><h2 className="text-lg font-bold">Corriger le stock</h2></div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button type="button" onClick={() => setMovementType('vente')} className={`rounded-xl border px-3 py-2 text-sm font-semibold ${movementType === 'vente' ? 'border-primary bg-primary text-primary-foreground' : 'border-border text-foreground hover:border-primary/50'}`}>Vendu</button>
            <button type="button" onClick={() => setMovementType('perte')} className={`rounded-xl border px-3 py-2 text-sm font-semibold ${movementType === 'perte' ? 'border-primary bg-primary text-primary-foreground' : 'border-border text-foreground hover:border-primary/50'}`}>Perdu / périmé / cassé</button>
          </div>
          {movementType === 'perte' && <Select value={reason} onValueChange={setReason}><SelectTrigger className="rounded-xl"><SelectValue placeholder="Précision optionnelle" /></SelectTrigger><SelectContent className="z-[120]">{lossReasons.map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select>}
          {movementType === 'perte' && reason === 'Autre' && <Input value={customReason} onChange={(e) => setCustomReason(e.target.value)} placeholder="Préciser la perte" className="rounded-xl" />}
          <Input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="rounded-xl" />
        </div>
        <div className="px-5 py-4 border-t border-border/40 flex justify-end gap-2"><Button variant="outline" onClick={onClose} className="rounded-xl">Annuler</Button><Button disabled={!canSave || saving} onClick={() => onSubmit({ product, quantity: Number(quantity), movementType, justification: justification.trim(), movementDate: date })} className="rounded-xl">Valider</Button></div>
      </div>
    </div>
  );
}